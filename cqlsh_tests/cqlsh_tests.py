...
class TestCqlsh(Tester):
    ...
    def test_sub_second_precision(self):
        ...
        output, err, rc = node1.run_cqlsh(cmds="use simple; SELECT * FROM testsubsecond "
...
UPDATE varcharmaptable SET varcharvarintmap['Vitrum edere possum, mihi non nocet.'] = 1010010101020400204143243 WHERE varcharkey= '᚛᚛ᚉᚑᚅᚔᚉᚉᚔᚋ ᚔᚈᚔ ᚍᚂᚐᚅᚑ ᚅᚔᚋᚌᚓᚅᚐ᚜'
    ...
    def test_unicode_syntax_error(self):
        ...
        output, err, rc = node1.run_cqlsh(cmds=u"ä;".encode('utf8'))
        err = err.decode('utf8')
    ...
    def test_unicode_invalid_request_error(self):
        ...
        cmd = cmd.encode('utf8')
        output, err, rc = node1.run_cqlsh(cmds=cmd)
...
class CqlshSmokeTest(Tester):
    ...
    def test_uuid(self):
        ...
        out, err, rc = self.node1.run_cqlsh("INSERT INTO ks.test (key) VALUES (uuid())")
        self.assertEqual(err, "")
    ...
    def test_commented_lines(self):
        ...
        self.node1.run_cqlsh(
             ...
             * comment */
            """)
        out, err, rc = self.node1.run_cqlsh("DESCRIBE KEYSPACE ks; // post-line comment")
        self.assertEqual(err, "")
    ...
    def test_select(self):
        ...
        out, err, rc = self.node1.run_cqlsh("SELECT key, c, v FROM ks.test")
    ...
    def test_use_keyspace(self):
        ...
        ks1_stdout, ks1_stderr, rc = self.node1.run_cqlsh(
            '''
            USE ks1;
            DESCRIBE TABLES;
            ''')
        ...
        ks2_stdout, ks2_stderr, rc = self.node1.run_cqlsh(
            '''
            USE ks2;
            DESCRIBE TABLES;
            ''')
...
class CqlLoginTest(Tester):
    ...
    def test_login_keeps_keyspace(self):
        ...
        cqlsh_stdout, cqlsh_stderr, rc = self.node1.run_cqlsh(
            '''
            USE ks1;
            DESCRIBE TABLES;
