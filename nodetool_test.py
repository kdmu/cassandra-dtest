...
class TestNodetool(Tester):
    ...
    def test_nodetool_timeout_commands(self):
        ...
        for timeout_type in types:
            out, err, rc = node.nodetool('gettimeout {}'.format(timeout_type))
            self.assertEqual(0, len(err), err)
            debug(out)
        ...
        for timeout_type in types:
            _, err, rc = node.nodetool('settimeout {} 123'.format(timeout_type))
            self.assertEqual(0, len(err), err)
    ...
    def test_meaningless_notice_in_status(self):
        ...
        out, err, rc = node.nodetool('status')
        self.assertEqual(0, len(err), err)
